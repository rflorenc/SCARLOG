...
from .data_loading import (
    ...
    estimate_contamination, enrich_log_entry_context,
    load_log_templates, load_unsw_features, resolve_dataset_path
)
...
class EvaluationEngine:
    ...
    def __init__(self, full_config: Optional[Dict[str, Any]] = None):
        ...
        reasoning_config = full_config.get('reasoning_enhancements', {})
        self.reasoning_enhancer = ReasoningEnhancer(reasoning_config) if reasoning_config else None
        self.reasoning_config = reasoning_config
        self.rag_config = full_config.get('rag_evaluation', {})
        self.full_config = full_config
        
    def _calculate_reasoning_sample_sizes(self, total_anomalies: int, total_normals: int) -> Tuple[int, int]:
        """
        Calculate balanced sample sizes for reasoning evaluation
        ...
        """
        # Get target sizes from configuration
        sampling_config = self.reasoning_config.get('sampling', {})
        target_anomalies = sampling_config.get('target_anomalies', 3)
        target_normals = sampling_config.get('target_normals', 3)
        
        # Don't exceed available samples
        anomaly_samples = min(target_anomalies, total_anomalies)
        normal_samples = min(target_normals, total_normals)
        
        # Ensure we have at least 1 of each if available
        if total_anomalies > 0 and anomaly_samples == 0:
            anomaly_samples = 1
        if total_normals > 0 and normal_samples == 0:
            normal_samples = 1
        
        logger.info(f"Calculated reasoning sample sizes: {anomaly_samples} anomalies (from {total_anomalies} available), {normal_samples} normals (from {total_normals} available)")
    ...
    async def run_unsw_nb15_evaluation(self, backend: InferenceBackend, train_path: str, 
        ...
        try:
            ...
            # Check if RAG evaluation is enabled
            if self.rag_config.get('enabled', False):
                ...
                logger.info("RAG evaluation is disabled in configuration")
    ...
    async def _run_reasoning_enhancement_evaluation(self, backend: InferenceBackend, 
        ...
        # Load template mappings for enrichment based on dataset type
        template_mapping = {}
        unsw_features = {}
        ...
        if dataset_type == "eventtraces":
            ...
            if os.path.exists(template_path):
                ...
                logger.info(f"Loaded {len(template_mapping)} HDFS templates for reasoning enhancement")
            else:
            ...
            if os.path.exists(template_path):
                template_mapping = load_log_templates(template_path)
                logger.info(f"Loaded {len(template_mapping)} BGL templates for reasoning enhancement")
            else:
            ...
            if os.path.exists(unsw_features_path):
                unsw_features = load_unsw_features(unsw_features_path)
                logger.info(f"Loaded {len(unsw_features)} UNSW-NB15 features for reasoning enhancement")
            else:
        ...
        for i, (log_entry, true_label) in enumerate(zip(test_lines, test_labels)):
            ...
            try:
                ...
                enhanced_result = await self.reasoning_enhancer.enhance_prediction(
                    backend, log_entry, dataset_type, template_mapping, unsw_features, approach_name
                )
    ...
    async def _run_reasoning_enhancement_evaluation_unlabeled(self, backend: InferenceBackend, 
        ...
        # Load template mappings for enrichment based on dataset type (same as labeled version)
        template_mapping = {}
        unsw_features = {}
        ...
        if dataset_type == "eventtraces":
            ...
            if os.path.exists(template_path):
                template_mapping = load_log_templates(template_path)
                logger.info(f"Loaded {len(template_mapping)} HDFS templates for reasoning enhancement (unlabeled)")
            else:
                logger.warning(f"HDFS template file not found for eventtraces dataset (unlabeled)")
            ...
            if os.path.exists(template_path):
                template_mapping = load_log_templates(template_path)
                logger.info(f"Loaded {len(template_mapping)} BGL templates for reasoning enhancement (unlabeled)")
            else:
                logger.warning(f"BGL template file not found for bgl dataset (unlabeled)")
            ...
            if os.path.exists(unsw_features_path):
                unsw_features = load_unsw_features(unsw_features_path)
                logger.info(f"Loaded {len(unsw_features)} UNSW-NB15 features for reasoning enhancement (unlabeled)")
            else:
                logger.warning(f"UNSW-NB15 features file not found at {unsw_features_path} (unlabeled)")
        else:
            logger.info(f"No specific enrichment available for dataset type: {dataset_type} (unlabeled)")
