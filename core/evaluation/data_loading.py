...
def resolve_dataset_path(path: str) -> str:
    """
    Resolve dataset path to absolute path, handling different working directories
    
    Args:
        path: Relative or absolute path to dataset
    ...
    script_dir = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))  # Go up from core/evaluation/
...
def load_unsw_features(features_path: str) -> Dict[str, str]:
    """
    Load UNSW-NB15 feature descriptions for enrichment
    
    Args:
        features_path: Path to UNSW-NB15_features.csv file
    ...
    try:
        ...
        for _, row in features_df.iterrows():
            ...
            feature_mapping[name] = f"{name}: {description}"
            
        logger.info(f"Loaded {len(feature_mapping)} UNSW-NB15 features from {features_path}")
        return feature_mapping
        
    except Exception as e:
        logger.warning(f"Could not load UNSW-NB15 features from {features_path}: {e}")
        return {}
...
def enrich_unsw_for_rag(text: str, feature_mapping: Dict[str, str]) -> str:
    """
    Enrich UNSW-NB15 network flow data with feature descriptions
    
    Args:
        text: Raw network flow data (CSV-like format)
        feature_mapping: Dictionary mapping feature names to descriptions
        
    Returns:
        Enriched text with feature explanations
    """
    if not feature_mapping:
        return text
    
    # For UNSW-NB15, the data is typically comma-separated values
    # We'll add contextual information about what each field represents
    enriched_parts = []
    
    # Add feature context if we can identify common network features
    if any(feature in text.lower() for feature in ['tcp', 'udp', 'icmp']):
        enriched_parts.append("Network protocol analysis:")
    
    # Add the original data
    enriched_parts.append(text)
    
    # Add relevant feature explanations for common network indicators
    relevant_features = []
    text_lower = text.lower()
    
    if 'tcp' in text_lower or 'udp' in text_lower or 'icmp' in text_lower:
        relevant_features.append("proto: Transaction protocol")
    if any(port in text for port in ['80', '443', '22', '445', '3389']):
        relevant_features.extend(["sport: Source port number", "dsport: Destination port number"])
    if 'syn' in text_lower or 'fin' in text_lower or 'rst' in text_lower:
        relevant_features.append("state: Connection state (ACC, CLO, CON, FIN, RST, etc.)")
    
    if relevant_features:
        enriched_parts.append("Key features: " + "; ".join(relevant_features))
...
def load_dataset(csv_path: str, dataset_type: str = "eventtraces", nlines: Optional[int] = None, preprocessing: Optional[Dict] = None) -> Tuple[List[str], np.ndarray]:
    ...
    Args:
        ...
        dataset_type: Type of dataset ("eventtraces", "bgl", "unsw-nb15")  
    ...
    """
    # Resolve dataset path to handle different working directories
    resolved_path = resolve_dataset_path(csv_path)
    logger.info(f"Loading {dataset_type} dataset from {resolved_path}, nlines={nlines}")
    
    if dataset_type == "eventtraces":
        return load_event_traces(resolved_path, nlines, preprocessing)
...
def load_event_traces(csv_path: str, nlines: Optional[int] = None, preprocessing: Optional[Dict] = None) -> Tuple[List[str], np.ndarray]:
    ...
    if preprocessing.get('enable_semantic_enrichment', False):
        ...
        # Resolve path to work from any working directory
        template_path = resolve_dataset_path(template_path)
    ...
    for idx, row in df.iterrows():
        ...
        if pd.notna(features):
            ...
            # Handle HDFS format with brackets
            ...
            if preprocessing.get('parse_features', True):
                # HDFS format - comma-separated tokens
    ...
    label_mapping = {"Success": 0, "Normal": 0, "Fail": 1, "Anomaly": 1, "0": 0, "1": 1}
    processed_labels = []
    for label in labels:
        if pd.notna(label):
...
def load_bgl_data(csv_path: str, nlines: Optional[int] = None, preprocessing: Optional[Dict] = None) -> Tuple[List[str], np.ndarray]:
    """
    Load BGL (BlueGene/L) dataset
    
    Args:
        csv_path: Path to CSV file with EventTrace and Label/binary_label columns
        nlines: Number of lines to load
        preprocessing: Dict of preprocessing options
        
    Returns:
        Tuple of (lines, labels) where lines are log content text
    """
    if not os.path.exists(csv_path):
        raise FileNotFoundError(f"Dataset file not found: {csv_path}")
    ...
    logger.info(f"Loaded BGL dataset: {len(df)} samples")
    
    preprocessing = preprocessing or {}
    
    # Load templates for semantic enrichment if needed
    template_mapping = {}
    if preprocessing.get('enable_semantic_enrichment', False):
        template_path = preprocessing.get('template_path', 
            'datasets/intermediate_tasks/task1_systems/loghub/BGL/BGL_processed/BGL_templates.csv')
        template_path = resolve_dataset_path(template_path)
        if os.path.exists(template_path):
            template_mapping = load_log_templates(template_path)
        else:
    ...
    for idx, row in df.iterrows():
        # BGL format uses EventTrace column
        features = row['EventTrace']
        
        # Handle both labeled and unlabeled BGL datasets
        if 'binary_label' in df.columns:
            label_val = row['binary_label']
        elif 'Label' in df.columns:
            label_val = row['Label']
        else:
            # Unlabeled case - use dummy label
            label_val = 0
        ...
        if pd.notna(features):
            ...
            if preprocessing.get('parse_features', True):
                # BGL format - log content as text
                tokens = raw_feat.split() if raw_feat else []
                
                if preprocessing.get('remove_empty_tokens', True):
                    tokens = [t for t in tokens if t != ""]
                
                # Apply semantic enrichment if enabled
                if template_mapping and preprocessing.get('enable_semantic_enrichment', False):
                    enriched_tokens = enrich_with_templates(tokens, template_mapping)
    ...
    for label in labels:
        if pd.notna(label):
            ...
            if isinstance(label, (int, float)):
                ...
                # BGL uses specific alert categories, map to binary
                if str(label) == "-":
                    mapped_label = 0  # Normal
                else:
                    mapped_label = 1  # Anomaly
            processed_labels.append(mapped_label)
        else:
            processed_labels.append(0)  # Default to normal for missing labels
...
def load_dataset_unlabeled(data_path: str, dataset_type: str = "hdfs", nlines: Optional[int] = None) -> Tuple[List[str], None, Optional[Dict]]:
    ...
    logger.info(f"Loading unlabeled {dataset_type} dataset from {resolved_path}")
    
    if dataset_type == "hdfs":
        return load_hdfs_unlabeled(resolved_path, nlines)
    elif dataset_type == "unsw-nb15":
        return load_unsw_nb15_unlabeled(resolved_path, nlines)
    else:
