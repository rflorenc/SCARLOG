    ...
    for clf_name, clf in classifiers.items():
        ...
        try:
            ...
            for metric in scoring:
                ...
                # Add individual CV score tracking for research reporting
                metrics[f'{metric}_cv_scores'] = test_scores.tolist()
            
            # Calculate average CV score (overall performance across all metrics)
            # Use F1 score as primary metric for average CV score
            metrics['avg_cv_score'] = np.mean(cv_results['test_f1'])
            metrics['avg_cv_score_std'] = np.std(cv_results['test_f1'])
            
            # Add cross-validation details for research reporting
            metrics['cv_folds'] = folds
            metrics['cv_method'] = 'StratifiedKFold'
            ...
            # Summary for this classifier - Enhanced for research reporting
            summary[clf_name] = {
                ...
                'avg_cv_score': metrics['avg_cv_score'],
                'avg_cv_score_std': metrics['avg_cv_score_std'],
            ...
            logger.info(f"{clf_name} - Accuracy: {metrics['accuracy_mean']:.3f} ± {metrics['accuracy_std']:.3f}, "
                       ...
                       f"Avg CV Score: {metrics['avg_cv_score']:.3f} ± {metrics['avg_cv_score_std']:.3f}")
...
def evaluate_extratrees_cv(X: np.ndarray, y: np.ndarray, n_splits: int = 5, 
    ...
    # Add average CV score (using F1 as primary metric for consistency)
    f1_values = [fold['f1'] for fold in fold_metrics]
    final_metrics['avg_cv_score'] = float(np.mean(f1_values))
    final_metrics['avg_cv_score_std'] = float(np.std(f1_values))
    
    # Add cross-validation details for research reporting
    final_metrics['cv_folds'] = n_splits
    final_metrics['cv_method'] = 'StratifiedKFold'
        ...
        for clf_name, clf_results in detailed_results.items():
            if 'error' not in clf_results:
                ...
                result = {
                    ...
                    'acc_std': clf_results['accuracy_std'],
                    'f1': clf_results['f1_mean'],
                    'f1_std': clf_results['f1_std'],
                    'precision': clf_results['precision_mean'],
                    'precision_std': clf_results['precision_std'],
                    'recall': clf_results['recall_mean'],
                    'recall_std': clf_results['recall_std'],
                    'roc_auc': clf_results['roc_auc_mean'],
                    'roc_auc_std': clf_results['roc_auc_std'],
                    'avg_cv_score': clf_results['avg_cv_score'],
                    'avg_cv_score_std': clf_results['avg_cv_score_std'],
                    'cv_folds': clf_results['cv_folds'],
                    'cv_method': clf_results['cv_method'],
                ...
                logger.info(f"{clf_name}: Acc={result['acc']:.3f}±{result['acc_std']:.3f}, "
                           f"F1={result['f1']:.3f}±{result['f1_std']:.3f}, "
                           f"Precision={result['precision']:.3f}±{result['precision_std']:.3f}, "
                           f"Recall={result['recall']:.3f}±{result['recall_std']:.3f}, "
                           f"AUC={result['roc_auc']:.3f}±{result['roc_auc_std']:.3f}, "
                           f"Avg CV={result['avg_cv_score']:.3f}±{result['avg_cv_score_std']:.3f}")
