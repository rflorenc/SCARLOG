...
def _process_deepseek_response(response: str) -> str:
    """
    Process DeepSeek model response. Since prompt is wrapped in <think> tags,
    the response should be clean without reasoning tokens.
    ...
    Returns:
        Cleaned response
    """
    # With proper prompt wrapping, response should be clean
    # Remove any stray thinking tokens that might appear
...
def rag_explanation_monitored(vector_db, query_text: str, model, tokenizer,
                            ...
                            dataset_type: str = "eventtraces", results_dir: str = "outputs",
                            template_mapping: Optional[Dict[str, str]] = None,
                            unsw_features: Optional[Dict[str, str]] = None,
                            model_name: Optional[str] = None) -> Tuple[str, str, List[int], List[Tuple], List[Tuple], Dict]:
    """
    ...
    Args:
        ...
        template_mapping: Template mapping for log enrichment
        unsw_features: UNSW features for network flow enrichment
        model_name: Name of the model being used for model-specific handling
    ...
    # Apply appropriate enrichment based on dataset type for prompt display only
    ...
    for doc, score in results:
        # Enrich retrieved logs for better context based on dataset type
        if dataset_type == "unsw-nb15" and unsw_features:
        ...
        retrieved_logs.append(enriched_log)
    ...
    # https://github.com/ray-project/llm-numbers?tab=readme-ov-file#40-901-amount-saved-by-appending-be-concise-to-your-prompt
    if structured:
        if dataset_type == "eventtraces":
            base_prompt = f"""You are a distributed file systems expert analyzing HDFS logs for anomaly detection.

New Log to Analyze: {enriched_query}
...
Be concise."""
            ...
            # For DeepSeek models, use standard prompt (thinking enforced in generation)
            prompt = base_prompt
                
        elif dataset_type == "bgl":
            base_prompt = f"""You are a systems expert analyzing BGL (Blue Gene/L) supercomputer logs for anomaly detection.
...
Rules:
- Focus on: ERROR/WARN/FATAL levels, node failures, memory errors, network issues, job failures
- Severity: 1=performance issue, 2=node instability, 3=critical system failure
- Common issues: hardware failures, memory corruption, network timeouts, job crashes
...
Be concise."""
            ...
            base_prompt = f"""You are a network security expert analyzing network flow data for intrusion detection.

New Network Flow to Analyze: {enriched_query}
...
Be concise."""
            ...
            # Generic prompt
            base_prompt = f"""Analyze this data for anomalies.

Data to Analyze: {enriched_query}

Retrieved Similar Examples:
{context_text}

Provide a detailed analysis explaining whether this data appears anomalous and why. Be concise."""
    ...
    try:
        ...
        # Process DeepSeek responses to strip reasoning tokens
        if is_deepseek:
            response = _process_deepseek_response(response)
...
class RAGEvaluationSystem:
    ...
    def setup_rag_database(self, lines: List[str], model, tokenizer, 
                          reducer=None, embedding_type: str = "llm", dataset_type: str = "eventtraces") -> bool:
        """Setup RAG database with embeddings"""
        try:
            # Load enrichment data based on dataset type
            if dataset_type == "eventtraces":
                # Load HDFS templates for eventtraces
                template_path = resolve_dataset_path("datasets/intermediate_tasks/task1_systems/loghub/HDFS/HDFS_v1_labelled/preprocessed/HDFS.log_templates.csv")
                if os.path.exists(template_path):
                    self.template_mapping = load_log_templates(template_path)
                    logger.info(f"Loaded {len(self.template_mapping)} HDFS templates from {template_path} for RAG enrichment")
                else:
                    logger.warning(f"HDFS template file not found for eventtraces dataset")
                    
            elif dataset_type == "bgl":
                # Load BGL templates for BGL dataset
                template_path = resolve_dataset_path("datasets/intermediate_tasks/task1_systems/loghub/BGL/BGL_processed/BGL_templates.csv")
                if os.path.exists(template_path):
                    self.template_mapping = load_log_templates(template_path)
                    logger.info(f"Loaded {len(self.template_mapping)} BGL templates from {template_path} for RAG enrichment")
                else:
                    logger.warning(f"BGL template file not found for bgl dataset")
                    
            elif dataset_type == "unsw-nb15":
                # Load UNSW-NB15 features for network flow enrichment
                unsw_features_path = resolve_dataset_path("datasets/intermediate_tasks/intrusion_detection/UNSW_NB15/UNSW-NB15_features.csv")
                if os.path.exists(unsw_features_path):
                    self.unsw_features = load_unsw_features(unsw_features_path)
                    logger.info(f"Loaded {len(self.unsw_features)} UNSW-NB15 features for RAG enrichment")
                else:
                    logger.warning(f"UNSW-NB15 features file not found at {unsw_features_path}")
            else:
                logger.info(f"No specific enrichment available for dataset type: {dataset_type}")
    ...
    def evaluate_with_rag(self, test_lines: List[str], test_indices: List[int],
                         ...
                         results_dir: str = "outputs", model_name: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        ...
        for idx in test_indices:
            if idx < len(test_lines):
                prompt, resp, ret_idx, pred_spans, pred_severity, rag_summary = rag_explanation_monitored(
                    ...
                    dataset_type=dataset_type, template_mapping=self.template_mapping,
                    unsw_features=self.unsw_features, model_name=model_name
                )
