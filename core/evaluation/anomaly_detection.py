...
def detect_anomalies_clustering(embeddings: np.ndarray, contamination: float = 0.1, 
    ...
    # PCA to 2D for clustering visualization only
    pca_variance_ratio = None
    if embeddings.shape[1] > 2:
        logger.info("Reducing dimensionality to 2D using PCA for visualization")
        pca = PCA(n_components=2, random_state=42)
        embeddings_2d = pca.fit_transform(embeddings)
        pca_variance_ratio = pca.explained_variance_ratio_
        logger.info(f"PCA explained variance ratio: {pca_variance_ratio}")
    else:
        embeddings_2d = embeddings.copy()
        pca_variance_ratio = np.array([1.0, 0.0])  # Full variance if already 2D
    ...
    try:
        ...
        # Cluster on full embeddings, not 2D reduction
    ...
    # DBSCAN clustering
    try:
        # Estimate eps using k-distance graph on full embeddings
