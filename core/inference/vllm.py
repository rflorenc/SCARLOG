...
# Fix MKL threading layer conflict before importing other libraries
...
class VLLMConfig(InferenceConfig):
    ...
    max_model_len: int = 128  # Match LocalTransformers default
...
class VLLMBackend(InferenceBackend):
    ...
    async def load_model(self) -> Dict[str, Any]:
        ...
        if self.is_large_model:
            ...
            memory_params = {
                ...
                "max_num_seqs": 1,
                "max_num_batched_tokens": self.config.max_model_len * 1  # Match max_model_len * max_num_seqs
            }
        else:
    ...
    async def generate_embeddings(self, texts: List[str], **kwargs) -> EmbeddingResult:
        ...
        max_length = kwargs.get('max_length', self.config.max_model_len)
        
        logger.info(f"Generating embeddings for {len(texts)} texts with vLLM (batch_size={batch_size}, max_length={max_length})")
        ...
        try:
            ...
            # Match LocalTransformers mean pooling for consistency
            try:
                # Try newer vLLM API first
                pooling_params = PoolingParams(pooling_type="mean")
            except TypeError:
                # Fall back to older vLLM API
                try:
                    pooling_params = PoolingParams(pooling_method="mean")
                except TypeError:
                    # Use default pooling if neither works
                    pooling_params = PoolingParams()
                    logger.warning("Using default pooling - unable to set mean pooling")
            all_embeddings = []
    ...
    async def unload_model(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"No distributed process group to destroy: {e}")
