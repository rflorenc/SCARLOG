...
echo "Running SCARLOG test with backend architecture"
...
run_and_log "eventtraces_lbl_${N}" \
  python ../run_benchmark.py --test-config test "$TRACES_LBL" ${N} eventtraces
done

UNSW_SIZES=(${MAIN_SIZE})              # edit e.g. (1000 5000 10000)
for N in "${UNSW_SIZES[@]}"; do
  run_and_log "unsw_lbl_${N}" \
    python ../run_benchmark.py --test-config test "$UNSW_TRAIN" "$UNSW_TEST" "$N" unsw-nb15
done
...
run_and_log "eventtraces_ul_${N}_auto" \
  python ../run_benchmark.py --test-config test "$TRACES_UL" ${N} eventtraces unlabeled auto
done

UNSW_SIZES=(${MAIN_SIZE})              # edit e.g. (1000 5000 10000)
for N in "${UNSW_SIZES[@]}"; do
run_and_log "unsw_ul_${N}_auto" \
  python ../run_benchmark.py --test-config test "$UNSW_UL" ${N} unsw-nb15 unlabeled auto
done
