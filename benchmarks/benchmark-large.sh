...
echo "Running SCARLOG large_models with backend architecture"
...
run_and_log "eventtraces_lbl_${N}k" \
  python ../run_benchmark.py --test-config large_models "$TRACES_LBL" ${N} eventtraces
done

UNSW_SIZES=(2000)              # edit e.g. (1000 5000 10000)
for N in "${UNSW_SIZES[@]}"; do
  run_and_log "unsw_lbl_${N}" \
    python ../run_benchmark.py --test-config large_models "$UNSW_TRAIN" "$UNSW_TEST" "$N" unsw-nb15
done

# BGL labeled benchmark
BGL_SIZES=(2000)              # edit e.g. (1000 5000 10000)
for N in "${BGL_SIZES[@]}"; do
  run_and_log "bgl_lbl_${N}" \
    python ../run_benchmark.py --test-config large_models "$BGL_LBL" ${N} bgl
done
...
run_and_log "eventtraces_ul_${N}k_auto" \
  python ../run_benchmark.py --test-config large_models "$TRACES_UL" ${N} eventtraces unlabeled auto
done

UNSW_SIZES=(2000)              # edit e.g. (1000 5000 10000)
for N in "${UNSW_SIZES[@]}"; do
run_and_log "unsw_ul_${N}_auto" \
  python ../run_benchmark.py --test-config large_models "$UNSW_UL" ${N} unsw-nb15 unlabeled auto
done

# BGL unlabeled benchmark
BGL_SIZES=(2000)              # edit e.g. (1000 5000 10000)
for N in "${BGL_SIZES[@]}"; do
run_and_log "bgl_ul_${N}_auto" \
  python ../run_benchmark.py --test-config large_models "$BGL_UL" ${N} bgl unlabeled auto
done

echo ""
echo "Benchmark large_models completed!"
