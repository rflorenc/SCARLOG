...
echo "Running SCARLOG small_models with backend architecture"
...
for N in "${BGL_SIZES[@]}"; do
  run_and_log "bgl_lbl_${N}" \
    python ../run_benchmark.py --test-config small_models "$BGL_LBL" ${N} bgl
done
...
run_and_log "bgl_ul_${N}_auto" \
  python ../run_benchmark.py --test-config small_models "$BGL_UL" ${N} bgl unlabeled auto
done
